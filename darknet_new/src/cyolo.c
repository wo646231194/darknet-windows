...
void train_cyolo(char *cfgfile, char *weightfile)
{
    ...
    char buff[256];
    sprintf(buff, "%s/cyolo_final.weights", backup_directory);
    save_weights(net, buff);
}

void convert_cyolo_detections(float *predictions, int classes, int num, int square, int side, int w, int h, float thresh, float **probs, box *boxes, int only_objectness)
{
    int i,j,n;
...
void validate_cyolo(char *cfgfile, char *weightfile)
{
    network net = parse_network_cfg(cfgfile);
    if(weightfile){
        load_weights(&net, weightfile);
...
void validate_cyolo_recall(char *cfgfile, char *weightfile)
{
    network net = parse_network_cfg(cfgfile);
    if(weightfile){
        load_weights(&net, weightfile);
    ...
    for(i = 0; i < m; ++i){
        ...
        convert_cyolo_detections(predictions, classes, l.n, square, side, 1, 1, thresh, probs, boxes, 1);
...
void test_cyolo(char *cfgfile, char *weightfile, char *filename, float thresh)
{
    ...
    while(1){
        ...
        convert_cyolo_detections(predictions, l.classes, l.n, l.sqrt, l.side, 1, 1, thresh, probs, boxes, 0);
...
void run_cyolo(int argc, char **argv)
{
    ...
    if(0==strcmp(argv[2], "test")) test_cyolo(cfg, weights, filename, thresh);
